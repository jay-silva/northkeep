...
if ! command -v gpg >/dev/null 2>&1; then
  ...
  # ONE KEY AT A TIME, best-effort. Asking for all 11 fingerprints in a single
  # --recv-keys call fails as a unit (measured: exit 2, ZERO keys imported) if
  # any one of them can't be served, which would fail every signed build. Note
  # keys.openpgp.org serves keys with no user ID and gpg then refuses to import
  # them, so in practice the second keyserver does the work; the fallback chain
  # is per key rather than per batch for the same reason.
  #
  # We do NOT need all 11 — only the SIGNER's key, and the ephemeral keyring is
  # what makes that safe: whatever lands here came from the pinned list this
  # run, so a successful verify proves the signer is currently pinned. Failing
  # to fetch an unrelated co-maintainer's key is not a reason to refuse.
  imported=0
  for _nk_key in "${NODE_RELEASE_KEYS[@]}"; do
    if gpg --batch --keyserver hkps://keys.openpgp.org --recv-keys "$_nk_key" >/dev/null 2>&1 \
       || gpg --batch --keyserver hkps://keyserver.ubuntu.com --recv-keys "$_nk_key" >/dev/null 2>&1; then
      imported=$((imported + 1))
    fi
  done
  echo "fetch-node: imported $imported/${#NODE_RELEASE_KEYS[@]} pinned release keys"
