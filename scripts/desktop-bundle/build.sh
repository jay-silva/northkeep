...
if [ -n "${APPLE_SIGNING_IDENTITY:-}" ] && { [ -n "${APPLE_PASSWORD:-}" ] || [ -n "${APPLE_API_KEY:-}" ]; }; then
  ...
  # unstapled and silently shipping an unnotarized disk image. Prefer the exact
  # version-matched file (version read from tauri.conf.json, no jq needed); fall
  # back to newest-by-mtime only if the version can't be read. Tauri names it
  ...
  if [ ! -e "${all_dmgs[0]}" ]; then
    # No DMG at all (e.g. a signed `--bundles app` smoke build). Nothing to
    # staple; leave DMG_OUT empty so the guard below skips the DMG steps.
    DMG_OUT=""
  elif [ -n "$VERSION" ]; then
  ...
  if [ -n "$DMG_OUT" ] && [ -n "$APP_OUT" ]; then
    ...
    if [ -n "${APPLE_API_KEY:-}" ] && [ -n "${APPLE_API_ISSUER:-}" ]; then
      ...
      # app-specific password on argv here — the key FILE is the credential, so
      # this is the preferred path; set APPLE_API_KEY_PATH/KEY/ISSUER in .env.local.
      ...
      # argv, but only for that one fast call; it writes to the login keychain
      # and overwrites the profile each run, so it stays idempotent.
