/**
 * Per-account request throttle (no dependencies, in-memory).
 *
 * Keyed by token hash when the request carries a bearer token, else by client
 * IP (covers the unauthenticated Stripe webhook path). State is per-process:
 * on Vercel each warm instance keeps its own window, so the effective global
 * ceiling is limit × instances — an intentional first line against a single
 * abusive account or a webhook flood, not a precise quota (KNOWN-LIMITS.md).
 *
...
/**
 * Read the per-window request cap from NORTHKEEP_SYNC_RATE_LIMIT.
