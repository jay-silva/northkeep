...
1. Plaintext memory content NEVER leaves the machine except to the model provider
   the user explicitly selected, after the active redaction tier has run.
2. Our sync server stores ciphertext only. No plaintext, no derived plaintext
   (no server-side embeddings, logs, or analytics on content).
