...
**Correction 2026-09-24 (release 0.22.0 doc-vs-code pass):** this work and its addenda are merged on main after v0.21.0 and ship for the first time in release 0.22.0. The owner's installed-app run of the d14fef5 build on 2026-09-12 found three defects (addendum below), all fixed in 48d2089. Later changes: 14bedcc implemented the owner's warm-on-open decision, and d4455b3 fixed the review-pass skip found on 2026-09-13. This repository records no passing installed-app acceptance of the fixed build, so installed-app acceptance is still pending.
...
### Owner decision 2026-09-12: warm on open

Shown the 20 s first search, the owner said: "Loading a local model is allowed, it would make sense for it to start running when you open NorthKeep." That supersedes the sentence in Decision above that forbade launching a model merely because a page loads, for the embedding model only. Implemented as `apps/web/src/search-warm.ts`: when `/api/status` sees an unlocked vault, a per-session warmer runs once. It reads local-search status; if the runtime is down and can be started it calls the same gated `/api/local/search/start` path the button uses (desktop macOS only, installed Ollama app, no arguments); if the embedding model is missing it stops and never downloads. It then opens the vault only to list live non-project contents and embeds each one through the cached embedder outside the vault lock, so sync and the MCP server are never blocked. It stops the moment the vault locks or the embedder fails, and backs off 60 s before another attempt. Nothing is written anywhere. Tests: `apps/web/test/search-warm.test.ts`.

### 2026-09-13: review pass skipped every memory

With the warm-on-open build installed, the owner ran Review on the personal collection and every memory was skipped. The saved report showed `coverage.skipped = 654` and `drops.embedding_unavailable = 654`: the pass ran without an embedder. Cause: `hasOllamaModel('nomic-embed-text')` in `@northkeep/librarian` accepted only the bare name or a hyphen-suffixed name, while Ollama 0.31 lists the untagged pull as `nomic-embed-text:latest`. This is exactly the mismatch the Decision above warned about ("the older helper is not used for this check") for search, but the review path and the CLI review command still used the older helper. Fixed in `d4455b3`: a bare tag also matches its `:latest` form; explicit tags remain exact. Regression test in `packages/librarian/test/review.test.ts`; live check against the owner's Ollama returned true for the bare name and resolved `qwen2.5:14b` for review.

### Owner decision 2026-09-13: agents search by meaning too

`memory_retrieve` in the local MCP server was keyword-only ("Dogs" found nothing while "dog" did) while the app searched by meaning. The owner asked for parity. The tool now calls `retrieveSemantic` with the same cached loopback embedder the app uses, memoized per server process, and reports `search_mode` ("semantic" or "keyword") plus the fallback reason in `note`. Candidates are embedded OUTSIDE the vault file lock before the ranking call, so the lock is never held across the loopback runtime (the ADR 0044 lesson); the standalone stdio server starts that pre-embedding at launch under the 2026-09-12 warm-on-open decision. `withVault` now awaits an async callback before closing the vault. The embedder resolves its loopback URL per call so tests can isolate it. The MCP server gains a workspace dependency on `@northkeep/librarian`; that package talks only to the loopback Ollama runtime the redaction tier already uses, so nothing new leaves the machine. KNOWN-LIMITS updated. Tests: `packages/mcp-server/test/server-tools.test.ts` (keyword with the runtime unreachable, semantic against a fake loopback embedder); the harness now points every test at a closed port by default so no test touches a real runtime.
...
**Correction 2026-09-24 (release 0.22.0 doc-vs-code pass):** 1a9e035 (2026-09-19) fixed that test. It now creates a locked vault and turns off the Keychain and environment key sources. It passes on this branch: `npx vitest run apps/web/test/api.test.ts -t "locked before checking"`, run under a temporary `NORTHKEEP_HOME`, reported 1 passed.
