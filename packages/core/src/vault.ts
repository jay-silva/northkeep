...
export class Vault {
  ...
  checkpointProject(request: ProjectCheckpointRequest, allowedScopes?: string[]): ProjectCheckpointResult {
    ...
    if (matches.length) {
      ...
      // Compaction blanked the base, so the content can no longer be rebuilt and
      // checked. The save did land; say so plainly instead of reporting a mismatch.
      if(base.forgotten_at!==null){let current:ProjectView|undefined;try{current=getProjectView(this,request.project,allowedScopes);}catch{}throw new ProjectHandoffError('stale_project','This save was already applied, and the version it started from has since been compacted, so it cannot be replayed. Read the project again; send any new save with a new operation id.',current);}
